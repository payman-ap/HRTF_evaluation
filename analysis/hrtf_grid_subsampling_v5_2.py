...
def greedy_angular_exclusion(candidates, alpha_deg=12.0, preserve_poles=True):
    ...
    azs = np.array([c['az'] for c in candidates])
    elevs = np.array([c['elev'] for c in candidates])
    ...
    if preserve_poles:
        for i, e in enumerate(elevs):
            if abs(abs(e) - 90.0) < 1e-6:
                selected.append(i)

    # Preserve cardinal azimuths per elevation
    for elev in np.unique(elevs):
        mask = np.isclose(elevs, elev)
        for ref in [0, 90, 180, 270]:
            idxs = np.where(mask & (circular_distance(azs, ref) < 1e-6))[0]
            if len(idxs) > 0:
    ...
    for i in range(len(candidates)):
        ...
        angs = np.rad2deg(np.arccos(np.clip(dots, -1.0, 1.0)))
        if np.min(angs) >= alpha_deg:
            selected.append(i)
...
def farthest_first(candidates, K=500, seed_index=0):
    ...
    while len(selected) < min(K, len(candidates)):
        mind = np.min(
            np.vstack([
                np.rad2deg(np.arccos(np.clip(units @ units[s], -1, 1)))
                for s in selected
            ]),
            axis=0
        )
...
# ------------------------- SH utilities & plotting remain unchanged -------------------------
