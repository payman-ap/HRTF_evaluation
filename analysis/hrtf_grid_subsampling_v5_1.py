...
CARDINAL_AZ = np.array([0.0, 90.0, 180.0, 270.0])
...
def subsample_azimuths(azimuths, min_sep, keep_ref=True):
    ...
    ref_angles = CARDINAL_AZ if keep_ref else []
...
def greedy_angular_exclusion(candidates, alpha_deg=12.0, preserve_poles=True):
    ...
    # Seed cardinal directions
    for i, c in enumerate(candidates):
        if any(np.isclose(c['az'], CARDINAL_AZ, atol=1e-6)):
            selected.append(i)
    ...
    for i in range(len(candidates)):
        ...
        angs = np.rad2deg(np.arccos(np.clip(dots, -1, 1)))
...
# (plotting, SH, diagnostics, main block)
